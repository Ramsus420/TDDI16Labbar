...
void AVL_Tree_Node<Comparable>::remove(const Comparable &x, Node_Pointer &t) {
    if (t == nullptr) {
        throw AVL_Tree_error("borttagning: ej funnen / nullptr");
    }
...
void AVL_Tree_Node<Comparable>::balance(Node_Pointer &t) {
    if (t == nullptr) {
        throw AVL_Tree_error("balansering: nullptr");
    }
    ...
    else if ((node_height(t->right) - node_height(t->left)) > 1) {
